...
<section class="shopping-cart">
   ...
   <form action="" method="POST" class="box">
      <a href="cart.php?delete=<?= $fetch_cart['id']; ?>" class="fas fa-times" onClick="return confirm('Hapus produk ini dari keranjang?');"></a>
      <a href="view_page.php?pid=<?= $fetch_cart['pid']; ?>" class="fas fa-eye"></a>
      <img src="uploaded_img/<?= $fetch_cart['image']; ?>" alt="">
      <div class="name"><?= $fetch_cart['name']; ?></div>
      <div class="price">RP. <?= $fetch_cart['price']; ?>/KG</div>
      <input type="hidden" name="cart_id" value="<?= $fetch_cart['id']; ?>">
      <div class="flex-btn">
         <input type="number" min="1" value="<?= $fetch_cart['quantity']; ?>" class="qty" name="p_qty">
         <input type="submit" value="ubah" name="update_qty" class="option-btn">
      </div>
   ...
   <div class="cart-total">
      <p>total bayar : <span>RP. <?= $grand_total; ?></span></p>
      <a href="shop.php" class="option-btn">Produk lain</a>
      <a href="cart.php?delete_all" class="delete-btn <?= ($grand_total > 1)?'':'disabled'; ?>">Hapus semua</a>
      <a href="checkout.php" class="btn <?= ($grand_total > 1)?'':'disabled'; ?>">Beli</a>
   </div>
